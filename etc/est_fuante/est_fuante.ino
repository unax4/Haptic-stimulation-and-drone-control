...
#include <MahonyAHRS.h> //pendiente de implementacion
...
unsigned long burstPause_ms = 500;
float freq_Hz = 100.0;
unsigned long pulseWidth_us = 400;
unsigned long trainDuration_ms = 3000;

// Analógicos para streaming
const int analogPins[4] = {A3, A2, A1, A0};

// Control de streaming
bool streamingEnabled = false; // Permite activar/desactivar streaming
    ...
    if (cmd.length() > 0) {
      ...
      if (cmd == "STOP") { 
        ...
        if (v >= 0 && v <= 255) { 
          potValue = v; 
          setPot(v);
          float intensidad_mA = 1/((0.0089 * potValue + 0.1505));
          float intensidad_mA_real = 1/((0.0102 * potValue + 0.175));
          Serial.print(F("Pot -> ")); Serial.print(potValue); Serial.print(F("/255 (~"));
          Serial.print(getPotResistance(), 0); Serial.print(F("Ω) -> Intensidad LMV342 ≈ "));
          Serial.print(intensidad_mA, 3); Serial.println(F(" mA"));
...
// --------------------
void sendToHV2701(uint16_t data) {
  spi_busy = true; // Bloquear streaming
