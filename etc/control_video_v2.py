...
class IMUAxisMapper:
    ...
    def __init__(self):
        self.deadzone=8.; self.sensitivity=1.; self.expo=.5
    ...
    def _a2s(self, angle):
        ...
        if mag<self.deadzone: return float(STICK_MID)
        norm=min(1.,(mag-self.deadzone)/(self.MAX_ANGLE-self.deadzone))
        norm=min(1.,norm*self.sensitivity); e=self.expo
...
class FlightController:
    ...
    def __init__(self, state: DroneState, log_q: queue.Queue):
        ...
        self.drone_ip=DEFAULT_IP; self.drone_port=DEFAULT_PORT; self.rate=80.
...
def _run_video_display(adapter: K417VideoAdapter):
    """
    Runs in its own daemon thread.  Opens a native OpenCV window and
    displays frames exactly as pruebas.py does.  Exits when the user
    presses Q or the adapter is stopped.
    """
    ...
    while adapter._running:
        # Non-blocking get — always show the most recently decoded frame
        # rather than waiting up to 1 s for the next one
        ...
        # waitKey(1) gives OpenCV ~1 ms to process window events — minimum needed
...
class K417GUI:
    ...
    def _build_sensitivity(self,parent):
        ...
        def param(label,from_,to,initial,setter,res=0.05):
            rw=tk.Frame(f,bg=PANEL_BG);rw.pack(fill="x",pady=3)
            tk.Label(rw,text=label,fg=TEXT_DIM,bg=PANEL_BG,font=FONT_LABEL,width=14,anchor="w").pack(side="left")
            var=tk.DoubleVar(value=initial)
            disp=tk.Label(rw,text=f"{initial:.2f}",fg=ACCENT,bg=PANEL_BG,font=FONT_MONO,width=5)
            disp.pack(side="right")
            def on(*_): v=round(var.get(),3);setter(v);disp.config(text=f"{v:.2f}")
            tk.Scale(rw,variable=var,from_=from_,to=to,orient="horizontal",resolution=res,length=180,
                     bg=PANEL_BG,fg=TEXT_MAIN,troughcolor=CARD_BG,highlightthickness=0,
                     activebackground=ACCENT,showvalue=False,command=on).pack(side="left",padx=4)
        param("Deadzone (°)",0.,30.,8.,lambda v:setattr(self.glove.mapper,"deadzone",v),res=0.5)
        param("Sensitivity",0.1,2.,1.,lambda v:setattr(self.glove.mapper,"sensitivity",v))
        param("Expo curve",0.,1.,.5,lambda v:setattr(self.glove.mapper,"expo",v))
        param("Thr smoothing",.02,.5,.12,lambda v:setattr(self.glove.mapper,"_throttle_alpha",v),res=0.01)
        param("Flex scale",20.,400.,IMUAxisMapper.FLEX_NORM_SCALE,
              lambda v:setattr(self.glove.mapper,"flex_norm_scale",v),res=5.)
        param("Mahony Kp",1.,15.,5.,lambda v:setattr(self.glove.ahrs,"kp",v),res=0.5)
    ...
    def _build_sticks(self,parent):
        ...
        tk.Label(f,text="LAST PACKET (hex):",fg=TEXT_DIM,bg=PANEL_BG,font=FONT_LABEL).grid(
            row=4,column=0,columnspan=3,sticky="w",padx=6,pady=(8,2))
        self._hex_label=tk.Label(f,text="—",fg=ACCENT,bg=CARD_BG,font=("Courier New",8),
                                  wraplength=320,justify="left",anchor="w",padx=4,pady=4)
        self._hex_label.grid(row=5,column=0,columnspan=3,sticky="ew",padx=6,pady=(0,6))
        bf=tk.Frame(f,bg=PANEL_BG);bf.grid(row=6,column=0,columnspan=3,sticky="ew",padx=6,pady=4)
    ...
    def _toggle_video(self):
        if self.video_adapter is not None:
            ...
            def _display_thread():
                _run_video_display(self.video_adapter)
    ...
    def _tick(self):
        ...
        preview=build_packet(int(snap["roll"]),int(snap["pitch"]),
                             int(snap["throttle"]),int(snap["yaw"]),
                             cmd,headless,0,0,0)
        ...
        for axis,val,rng in [("YAW",g.yaw_deg,180.),("PITCH",g.pitch_deg,90.),("ROLL",g.roll_deg,90.)]:
            ...
            dz=self.glove.mapper.deadzone;dz_px=int((dz/rng)*(w//2))
