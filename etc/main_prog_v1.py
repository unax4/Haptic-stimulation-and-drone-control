...
class HandSimulator:
    ...
    def reset_orientation(self):
            gui_print("[INFO] Reseteando orientación (mano plana en mesa)...")
    ...
    def process_sensor_data(self, line):
        ...
        accel_roll = np.arctan2(self.ay_prev, self.az_prev)
        accel_pitch = np.arctan2(-self.ax_prev, np.sqrt(self.ay_prev**2 + self.az_prev**2))
        
        self.roll = (1 - self.complementary_alpha) * (self.roll + gx_rad * dt) + self.complementary_alpha * accel_roll
        self.pitch = (1 - self.complementary_alpha) * (self.pitch + gy_rad * dt) + self.complementary_alpha * accel_pitch
        ...
        target_orn = p.getQuaternionFromEuler([self.roll, self.pitch, self.yaw_f])
