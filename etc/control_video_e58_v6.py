...
def build_packet(roll, pitch, throttle, yaw, command, headless, c1, c2, c3,
    ...
    chk = roll_i ^ pitch_i ^ throttle_i ^ yaw_i ^ cmd
    return bytes((0x66, roll_i, pitch_i, throttle_i, yaw_i, cmd, chk & 0xFF, 0x99))
...
class DroneState:
    ...
    def consume_flags(self):
        with self._lock:
            ...
            elif self.headless_flag:  cmd,self.headless_flag   =HEADLESS_ON,  False
...
class WifiCamVideoAdapter:
    ...
    def _extract_jpegs(self, payload):
        start = payload.find(_SOI)
        end = payload.find(_EOI)
        if start != -1 and end != -1 and end > start:
            self._emit_jpeg(payload[start:end + 2])
            return

        self._jpeg_buf.extend(payload)
        if len(self._jpeg_buf) > (3 * 1024 * 1024):
            self._jpeg_buf = self._jpeg_buf[-1024 * 1024:]

        while True:
            s = self._jpeg_buf.find(_SOI)
            if s == -1:
                if len(self._jpeg_buf) > 1024:
                    self._jpeg_buf = self._jpeg_buf[-1024:]
                break
            e = self._jpeg_buf.find(_EOI, s + 2)
            if e == -1:
                if s > 0:
                    del self._jpeg_buf[:s]
                break
            self._emit_jpeg(bytes(self._jpeg_buf[s:e + 2]))
            del self._jpeg_buf[:e + 2]
...
class K417GUI:
    ...
    def _toggle_headless(self):
        ...
        self._btn_head.config(bg=ACCENT, fg=DARK_BG)
        self.root.after(180, lambda: self._btn_head.config(bg=BTN_HEAD, fg="white"))
        self._log_event("CMD: HEADLESS")
    ...
    def _toggle_video(self):
        if self.video_adapter is not None:
            ...
            self.video_adapter = WifiCamVideoAdapter(drone_ip=ip, port=port)
            self.ctrl.inject_socket(self.video_adapter._sock)
            self.ctrl.send_start_control(burst=4)
            self.ctrl.start()
    ...
    def _start_control(self):
        ...
        self._log_event("START sent, control loop running")
