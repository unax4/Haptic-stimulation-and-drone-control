...
──────────────────────────────────────────────────────────────
The original single-shared-socket design caused video freezes:
  • FlightController._send() and K417VideoAdapter._rx_loop()
    raced on the same non-blocking socket.
  • Watchdog and warmup threads also called sendto() concurrently.

Fixed design — two completely independent sockets:

  Socket A  (video socket, K417VideoAdapter)
    • Created by VideoAdapter, bound to a random local port.
    • BLOCKING with 10 ms select() timeout.
    • Receives all video UDP datagrams from the drone.
    • Sends START_STREAM, REQUEST_A, REQUEST_B via an internal
      send-queue (single writer thread) to eliminate send/recv races.
    • The drone learns this port from START_STREAM and streams
      video frames back to it for the entire session.

  Socket B  (control socket, FlightController)
    • Plain unbound UDP socket, send-only.
    • FlightController owns it exclusively — no sharing ever.
    • Sends all RC control packets at 40 Hz.
    • The drone does NOT need to reply to this port.

  TelemetryParser is fed from Socket A (non-video UDP replies
  arrive on the same port as video frames).
...
CMD_STOP      = 0x02
CMD_CALIBRATE = 0x04
CMD_CAM_UP    = 0x05   # camera tilt up
CMD_CAM_DOWN  = 0x06   # camera tilt down
HEADLESS_OFF  = 0x02
HEADLESS_ON   = 0x03
...
def build_packet(roll, pitch, throttle, yaw, command, headless, c1, c2, c3,
    ...
    """
    Build a K417 control packet.
    somersault_flag=True adds 0x08 to the headless byte to trigger a flip.
    The flip direction is encoded in the roll/pitch sticks at call time.
    """
    b_c1 = c1.to_bytes(2,"little"); b_c2 = c2.to_bytes(2,"little"); b_c3 = c3.to_bytes(2,"little")
    headless_byte = headless & 0xFF
    if somersault_flag:
        headless_byte |= _SOMERSAULT_FLAG
    controls = [roll&0xFF, pitch&0xFF, throttle&0xFF, yaw&0xFF, command&0xFF, headless_byte]
    chk = 0
...
# ──────────────────────────────────────────────────────────────────────────────
# FlightController  — dedicated send-only socket, never shared with video
# ──────────────────────────────────────────────────────────────────────────────
class FlightController:
    """
    Sends UDP control packets at a fixed rate.

    Owns its own dedicated send-only UDP socket.  This socket is never
    shared with the video adapter — the two-socket architecture means
    there is zero contention between control TX and video RX.
    """
    ...
    def _open_sock(self):
        ...
        # Send buffer 64 KB is plenty for 40 Hz control packets (~120 bytes each)
    ...
    def _loop(self):
        ...
        try:
            while self._running:
                ...
                # ── smart landing ────────────────────────────────────────────
                if self.state.smart_land_active:
                    self._do_smart_land()
                    continue
    ...
    # ── camera tilt (burst of CMD_CAM_UP/DOWN packets) ────────────────────────
    def _send_camera_tilt(self, direction: str):
        ...
        for _ in range(8):
            ...
            self._send(pkt)
            time.sleep(0.025)
        try: self.log_q.put_nowait(f"CAM: tilt {direction}")
        except queue.Full: pass
    ...
    def _do_flip(self):
        ...
        if direction == FLIP_FORWARD:
            flip_pitch, flip_roll = STICK_MAX, STICK_MID
        elif direction == FLIP_BACKWARD:
            flip_pitch, flip_roll = STICK_MIN, STICK_MID
        elif direction == FLIP_RIGHT:
            flip_pitch, flip_roll = STICK_MID, STICK_MAX
        elif direction == FLIP_LEFT:
            flip_pitch, flip_roll = STICK_MID, STICK_MIN
        else:
            flip_pitch, flip_roll = STICK_MAX, STICK_MID
        ...
        try: self.log_q.put_nowait(f"FLIP: {direction} — done")
        except queue.Full: pass

    # ── smart landing ─────────────────────────────────────────────────────────
    def _do_smart_land(self):
        LAND_ALTITUDE_CM = 20     # cut motors when altitude ≤ this
        DESCENT_THROTTLE = STICK_MID - 30
        MAX_DESCENT_S    = 8.0
        TIMED_RAMP_S     = 4.0
        ...
        def _send_thr(thr, cmd=CMD_NONE):
            c1, c2, c3 = self.state.next_counters()
            self._send(build_packet(STICK_MID, STICK_MID, int(thr),
                                    int(snap["yaw"]), cmd, headless, c1, c2, c3))

        try: self.log_q.put_nowait("LAND: smart descent starting…")
        except queue.Full: pass
        ...
        if tel is not None:
            while self._running:
                alt = tel.snapshot().get("altitude_cm", -1)
                if 0 <= alt <= LAND_ALTITUDE_CM:
                    break
                if time.time() - t_start > MAX_DESCENT_S:
                    try: self.log_q.put_nowait("LAND: timeout — cutting motors")
                    except queue.Full: pass
                    break
                _send_thr(DESCENT_THROTTLE)
                time.sleep(1. / self.rate)
        else:
            steps     = int(TIMED_RAMP_S * self.rate)
            thr_range = snap["throttle"] - STICK_MIN
            for i in range(steps):
                if not self._running: break
                thr = snap["throttle"] - (i / max(steps - 1, 1)) * thr_range
                _send_thr(max(STICK_MIN, thr))
                time.sleep(1. / self.rate)

        for _ in range(10):
            _send_thr(STICK_MIN, CMD_LAND)
            time.sleep(0.02)

        self.state.smart_land_active = False
        try: self.log_q.put_nowait("LAND: motors stopped ✓")
        except queue.Full: pass
    ...
    def stop(self):
        ...
        if self._thread:
            ...
            self._thread = None
        # _close_sock() is called in the finally block of _loop;
        # call it here too in case start() was never called.
        self._close_sock()
...
# ──────────────────────────────────────────────────────────────────────────────
# K417VideoAdapter  — dedicated video socket, internal send-queue, no sharing
# ──────────────────────────────────────────────────────────────────────────────
import ctypes
import sys
...
class K417VideoAdapter:
    """
    Handles the WiFi-UAV/K417 proprietary UDP video stream.

    Socket design (two-socket architecture):
      • This class owns ONE blocking UDP socket bound to a random port.
      • That socket is used ONLY for video: receiving frame fragments and
        sending START_STREAM / REQUEST_A / REQUEST_B.
      • All sends go through an internal _send_q (a Queue) and are executed
        by a single dedicated _tx_loop thread.  This means the RX thread
        (_rx_loop using select) never races with any sender.
      • FlightController has its own completely independent socket and is
        never injected into or referenced by this class.

    Telemetry:
      • Non-video UDP replies from the drone arrive on the same port.
      • _rx_loop calls _telemetry.ingest() for every such packet if a
        TelemetryParser has been wired in via set_telemetry().
    """

    HEADER_LEN    = 56
    FRAME_TIMEOUT = 0.08   # s — resend frame request after this idle period
    MAX_RETRIES   = 3
    WATCHDOG_SLEEP = 0.05  # s

    # sentinel that tells _tx_loop to exit cleanly
    _TX_STOP = object()
    ...
    def __init__(self, drone_ip=DEFAULT_IP, port=DEFAULT_PORT,
        ...
        self._telemetry: TelemetryParser | None = None

        # ── video socket: blocking, large recv buffer ─────────────────────
        self._sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self._sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        self._sock.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 524288)   # 512 KB
        self._sock.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 65536)    # 64 KB
        self._sock.bind(("", 0))
        # Blocking socket — _rx_loop uses select() so it never hard-blocks
        self._sock.setblocking(True)

        # ── internal send-queue: all outbound video-socket writes go here ─
        self._send_q: queue.Queue = queue.Queue(maxsize=32)
        ...
        # Start threads: TX first so the socket is ready to send before RX fires
        threading.Thread(target=self._tx_loop,      daemon=True, name="K417-TX").start()
        ...
        # Prime the stream
        self._enqueue(START_STREAM)
        self._enqueue_frame_request(0)

    # ── public API ────────────────────────────────────────────────────────────
    def set_telemetry(self, tel: "TelemetryParser"):
        """Wire in the shared TelemetryParser so drone replies are parsed."""
        self._telemetry = tel

    def get_frame(self, timeout=0):
        try:
            return self._frame_q.get(timeout=timeout) if timeout > 0 \
                   else self._frame_q.get_nowait()
        except queue.Empty:
            return None
    ...
    def stop(self):
        self._running = False
        # Unblock _tx_loop
        try: self._send_q.put_nowait(self._TX_STOP)
        except queue.Full: pass
        # Close socket — unblocks any pending select() in _rx_loop
        try: self._sock.close()
        except Exception: pass

    # ── send-queue helpers ────────────────────────────────────────────────────
    def _enqueue(self, data: bytes):
        """Queue a raw packet for transmission.  Non-blocking; drops if full."""
        try: self._send_q.put_nowait(data)
        except queue.Full: pass

    def _enqueue_frame_request(self, frame_id: int):
        ...
        for base in (12, 88, 107):
            rq_b[base] = lo; rq_b[base + 1] = hi
        self._enqueue(bytes(rq_a))
        self._enqueue(bytes(rq_b))
        self._last_req_ts = time.time()

    # ── TX thread — sole writer to the video socket ───────────────────────────
    def _tx_loop(self):
        """
        Single writer thread.  Dequeues packets and sends them.
        Being the only writer means no send/recv locking is ever needed.
        """
        while True:
            try:
                ...
                if not self._running:
                    break
                continue
            if item is self._TX_STOP:
                break
            try:
                self._sock.sendto(item, (self.drone_ip, self.port))
            except OSError:
                if self._running:
                    time.sleep(0.005)

    # ── RX thread — sole reader from the video socket ─────────────────────────
    def _rx_loop(self):
        """
        Blocking receive loop using select() with a 10 ms timeout so the
        thread can notice _running=False without needing the socket closed.
        This thread never calls sendto() — all sends go through _enqueue().
        """
        import select
        while self._running:
            try:
                ...
                # Socket closed or invalid — exit cleanly
                break

            # Route to telemetry parser first (non-video replies)
            if self._telemetry is not None:
                self._telemetry.ingest(payload)
    ...
    def _handle_payload(self, payload: bytes):
        ...
        # Push to display queue (drop stale frame if full)
        ...
        self._fragments.clear()
        self._enqueue_frame_request(frame_id)
        self._current_fid = (frame_id + 1) & 0xFFFF

    # ── watchdog thread ───────────────────────────────────────────────────────
    def _watchdog_loop(self):
        """Re-request the current frame if no fragment arrives within FRAME_TIMEOUT."""
        while self._running:
            time.sleep(self.WATCHDOG_SLEEP)
            if time.time() - self._last_req_ts < self.FRAME_TIMEOUT:
                continue
            if self._retry_cnt < self.MAX_RETRIES:
                self._enqueue_frame_request((self._current_fid - 1) & 0xFFFF)
                self._retry_cnt += 1
            else:
                self.frames_dropped += 1
                self._fragments.clear()
    ...
    def _warmup_loop(self):
        """Keep hammering START_STREAM until the first frame arrives."""
...
class K417GUI:
    def __init__(self,root):
        ...
        # FlightController still gets a telemetry ref for smart-land altitude checks.
        # The actual data is populated by K417VideoAdapter._rx_loop (not shared socket).
    ...
    def _cmd_land(self):
        """Smart landing: ramp throttle down using altitude telemetry, then cut motors."""
        if self.state.smart_land_active:
            return  # already landing
        self.state.smart_land_active=True
        self._log_event("CMD: SMART LAND — descending…")
    ...
    def _toggle_video(self):
        if self.video_adapter is not None:
            # ── STOP ──────────────────────────────────────────────────────
            # VideoAdapter owns its own socket — just stop it.
            # FlightController has its own independent socket and keeps running.
            ...
            # ── START ─────────────────────────────────────────────────────
            # Two-socket architecture: VideoAdapter creates its own socket
            # and sends START_STREAM from it.  FlightController already has
            # its own independent socket running — no cycling, no injection.
            # The drone accepts control from one port and video-req from another.
            self.video_adapter = K417VideoAdapter(drone_ip=ip, port=port)
            # Wire telemetry so the adapter feeds battery/altitude data
            self.video_adapter.set_telemetry(self.telemetry)

            # Make sure FlightController is running and aimed at the right target
    ...
    def _on_video_closed(self):
        """Called from the main thread when the OpenCV window is closed by the user."""
        # VideoAdapter has already stopped itself (adapter.stop() called from display thread).
        # FlightController keeps running uninterrupted — its socket is independent.
        self.video_adapter = None
        self._video_thread = None
        self._video_btn.config(text="▶  START VIDEO", bg="#005f73")
        self._status_label.config(text="● CONNECTED", fg=BTN_TAKE)
