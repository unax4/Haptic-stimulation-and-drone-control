...
# --- NUEVAS IMPORTACIONES PARA MEDIAPIPE ---
import cv2
...
# ==============================================================================
# CLASE MEDIAPIPE: TRACKING DE POSICIÓN (VISIÓN ARTIFICIAL)
# ==============================================================================
class MediaPipePosTracker:
    """Clase para detectar la posición de la mano usando Webcam y MediaPipe."""
    def __init__(self):
        self.mp_hands = mp.solutions.hands
        # max_num_hands=1 para evitar saltos entre dos manos
        ...
        self.cap = None
        self.position = [0.0, 0.0, 0.3]  # Posición inicial [X, Y, Z]
        self.hand_detected = False
        self.running = True
        
        # Factores de escala para convertir pantalla (0-1) a PyBullet (Metros)
        self.scale_x = 1.5  # Sensibilidad X
        self.scale_z = 1.0  # Sensibilidad Altura (Z en PyBullet, Y en Pantalla)
        self.scale_depth = 1.5 # Sensibilidad Profundidad
        
        # Offsets para centrar la mano
        self.offset_x = -0.75
        self.offset_y = 0.2  # Profundidad base
        self.offset_z = 0.5  # Altura base
    ...
    def start(self):
        ...
        try:
            ...
            if not self.cap.isOpened():
                gui_print("[VISION] Error: No se detectó Webcam.")
                return
            
            gui_print("[VISION] Webcam iniciada. Usando MediaPipe para posición XYZ.")
            t = threading.Thread(target=self._loop, daemon=True)
            t.start()
        except Exception as e:
            gui_print(f"[VISION] Error iniciando cámara: {e}")
        ...
        while self.running and self.cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                ...
                for hand_landmarks in results.multi_hand_landmarks:
                    # Usamos la muñeca (Landmark 0) como punto de anclaje
                    wrist = hand_landmarks.landmark[0]
                    
                    # --- MAPEO DE COORDENADAS ---
                    # MediaPipe: X(0 izq -> 1 der), Y(0 arriba -> 1 abajo), Z(relativo)
                    # PyBullet: X(lat), Y(profundidad), Z(altura)
                    
                    # X: Invertir si es necesario (modo espejo)
                    pb_x = (wrist.x * -self.scale_x) - self.offset_x
                    
                    # Z (Altura): Invertir Y de MediaPipe (0 es arriba en pantalla, pero alto en 3D)
                    pb_z = ((1.0 - wrist.y) * self.scale_z) - 0.2
                    
                    # Y (Profundidad): Usamos el tamaño de la mano o Z relativo. 
                    # Una aproximación simple es usar Z relativo de MP o una posición fija si falla.
                    # Aquí usamos el Z de MP que es profundidad relativa a la muñeca, 
                    # pero para mover la mano adelante/atrás, lo mejor es acercar la mano a la cámara.
                    # MediaPipe Z es difícil de usar para posición absoluta. 
                    # Truco: Usar el tamaño aparente o simplemente mapear Z relativo con un offset.
                    pb_y = (wrist.z * -self.scale_depth) + self.offset_y

                    # Actualizar posición (suavizado simple puede ir aquí)
                    self.position = [pb_x, pb_y, pb_z]
...
class IMUSensorFusion:
    """Clase para la fusión de sensores IMU usando filtro Madgwick con calibración y offset para reset de orientación."""
    def __init__(self):
    ...
    def update(self, ax, ay, az, gx, gy, gz, dt):
        """Actualiza el cuaternión usando datos del acelerómetro y giroscopio (filtro Madgwick).
...
class HandSimulator:
    def __init__(self, urdf_filename="schunk_svh_hand_right.urdf"):
        ...
        # --- IMU: Usamos la clase de fusión de sensores ---
        self.imu = IMUSensorFusion()
        self.calibration_steps = 150  # Número de muestras para calibrar el IMU
        
        # --- TRACKER MEDIAPIPE ---
        self.mp_tracker = MediaPipePosTracker()
        self.mp_tracker.start()
        ...
        # Teclas para mover la base de la mano (Backup si falla la cámara)
        self.key_map = {
            ord('1'): (1, 0, 0),    # +X
            ord('2'): (-1, 0, 0),   # -X
            ord('3'): (0, 1, 0),    # +Y
    ...
    def process_sensor_data(self, line):
        ...
        # --- ACTUALIZAR POSICIÓN Y ORIENTACIÓN EN PYBULLET ---
        
        # 1. Obtener POSICIÓN desde MediaPipe (si hay mano)
        mp_pos, is_hand_visible = self.mp_tracker.get_position()
        
        if is_hand_visible:
            # Si MediaPipe ve la mano, usas esa posición (la cámara manda)
            # Aplicamos algo de suavizado directo en process_sensor_data también
            target_pos = np.array(mp_pos)
            # Actualizamos self.base_target_pos para que la transición sea suave si se pierde la cámara
            self.base_target_pos = target_pos 
        else:
            # Si no hay cámara, usar teclado (modo manual)
            self.base_target_pos = np.array(self.base_target_pos) + np.array(self.translation_vector)
            target_pos = self.base_target_pos
        
        # 2. Obtener ORIENTACIÓN desde el IMU
        ...
        # Suavizado dinámico
        # Si estamos usando MediaPipe, reducimos un poco el suavizado para menos latencia
        smooth = 0.5 if is_hand_visible else self.smoothing_factor
        
        new_pos = np.array(curr_pos) * (1 - smooth) + target_pos * smooth
        new_orn = p.getQuaternionSlerp(curr_orn, target_orn, smooth)
    ...
    def visualization_loop(self):
        ...
        gui_print("="*60)
        gui_print("\n--- CONTROLES ---")
        gui_print("CÁMARA: Mueve tu mano real frente a la webcam para posición XYZ")
        ...
        gui_print("Teclas 1-6: Mover mano en XYZ (si no hay cámara)")
...
if __name__ == "__main__":
    ...
    def send_command(event):
        ...
        if cmd_lower == "quit":
            global running
            running = False
            # Detener también el tracker de visión
            if hasattr(sim, 'mp_tracker'):
                sim.mp_tracker.running = False
            root.quit()
            return
