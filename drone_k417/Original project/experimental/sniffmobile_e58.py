#!/usr/bin/env python3
"""
Capture mobile->drone UDP traffic from KY UFO (K417) while the phone is connected through the PC hotspot.

This script is designed for the workflow you described:
1) Phone connected to Windows hotspot
2) Open KY UFO app and press controls
...
Examples:
  python experimental/sniffmobile_k417.py --list-ifaces

  python experimental/sniffmobile_k417.py \
    --iface "Wi-Fi" --phone-ip 192.168.137.195 --drone-ip 192.168.4.153 --ports 8080,8090
& c:\python311\python.exe "c:/Users/Unax/Desktop/Legacy Code/drone_k417/Original project/experimental/sniffmobile_k417.py" --iface "Wi-Fi" --no-bpf --debug-udp-preview 50 --ports 8080,8090
Output files (default):
- captures/k417_mobile_to_drone_events.jsonl
- captures/k417_mobile_to_drone_unique.json
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-events",
        type=str,
        default="captures/k417_mobile_to_drone_events.jsonl",
        help="Path to JSONL output for packet events",
    )
    parser.add_argument(
        "--out-unique",
        type=str,
        default="captures/k417_mobile_to_drone_unique.json",
...
def xor5_ok(payload: bytes) -> bool:
    """Check candidate 8-byte K417 WIFI CAM control frame checksum.
...
def parse_k417_control(payload: bytes):
    """Parse K417 control packet if present.

    Returns dict with fields or None when not a K417 control packet.
    """
    # K417 control packet prefix (from control_video_v7 build_packet)
    K417_HDR = bytes([0xEF, 0x02, 0x7C, 0x00])
    if not payload.startswith(K417_HDR) or len(payload) < 37:
        return None

    # Layout (based on build_packet):
    # 0..11: _HDR (12 bytes)
    # 12..13: c1 (2 bytes little-endian)
    # 14..19: _C1_SUFFIX (6 bytes)
    # 20..25: controls: roll,pitch,throttle,yaw,command,headless
    # 26..35: _CTRL_PAD (10 bytes)
    # 36: checksum
    try:
        c1 = int.from_bytes(payload[12:14], "little")
        controls = payload[20:26]
        roll, pitch, throttle, yaw, cmd, headless = (b for b in controls)
        chk = payload[36]
        # compute checksum (xor of control bytes)
        calc = 0
        for b in controls:
            calc ^= b
        chk_ok = calc == chk
...
def main() -> None:
    ...
    with open(args.out_events, "w", encoding="utf-8") as event_fp:
        ...
        def on_packet(pkt) -> None:
            ...
            k417 = parse_k417_control(payload)
            event = {
                "ts": now,
                "src_ip": ip.src,
            ...
            if k417 is not None:
                label = "K417_CTRL"
                cmd_name = f"0x{k417['cmd']:02X}"
                somersault = bool(k417["headless"] & 0x08)
                print(
                    f"[{total:06d}] {label} {ip.src}:{udp.sport} -> {ip.dst}:{udp.dport} "
                    f"len={len(payload)} roll={k417['roll']} pitch={k417['pitch']} "
                    f"thr={k417['throttle']} yaw={k417['yaw']} cmd={cmd_name} "
                    f"headless=0x{k417['headless']:02X} somersault={somersault} "
                    f"chk={'OK' if k417['checksum_ok'] else 'BAD'}"
                )
            else:
