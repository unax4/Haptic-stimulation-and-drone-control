...
IMU (Arduino Nano RP2040):
    - Mahony AHRS + local sensor processing run on Arduino
    - Python receives yaw/pitch/roll + throttle telemetry via serial

Serial data format:
    Y:<yaw> P:<pitch> R:<roll> T:<thr> A0:<raw> A1:<raw> A2:<raw> A3:<raw> POS:<int>
...
class FlightController:
    ...
    Video mode: shared socket injected by K417VideoAdapter so control and
    video traffic use the same source port (required by K417 in many cases).
    """
    ...
    def inject_socket(self, sock: socket.socket):
        """Called by K417VideoAdapter to share the video socket for control."""
    ...
    # ── smart landing helper ──────────────────────────────────────────────
    def _do_smart_land(self):
        """
        Gradual-descent landing:
        1. If the TelemetryParser is available and reports altitude,
           ramp throttle down until altitude ≤ LAND_ALTITUDE_THRESHOLD_CM,
           then send CMD_LAND to cut motors cleanly.
        2. If no telemetry is available, do a timed ramp-down (safe fallback).

        Altitude telemetry note:
        The TelemetryParser reads altitude_cm from the drone's UDP replies
        (payload[6]).  Many cheap WiFi-UAV drones only update this byte when
        airborne and it may read 0 on the ground, which is our landing trigger.
        """
        LAND_ALTITUDE_THRESHOLD_CM = 20   # consider "near ground" at ≤20 cm
        DESCENT_THROTTLE  = STICK_MID - 30  # ~98 — enough downward thrust
        HOLD_THROTTLE     = STICK_MID       # neutral when waiting for telemetry
        MAX_DESCENT_S     = 8.0             # never descend for longer than this
        TIMED_RAMP_S      = 4.0             # fallback ramp duration if no telemetry

        _,headless = self.state.consume_flags()
        snap = self.state.snapshot()

        def _send_throttle(thr, cmd=CMD_NONE):
            c1,c2,c3 = self.state.next_counters()
            pkt = build_packet(STICK_MID, STICK_MID, int(thr),
        ...
        # Check whether telemetry data is available (injected by GUI)
        tel = getattr(self, "_telemetry", None)
        use_telemetry = tel is not None

        t_start = time.time()
        if use_telemetry:
            # ── telemetry-guided descent ──────────────────────────────────
            while self._running:
                elapsed = time.time() - t_start
                data = tel.snapshot()
                alt = data.get("altitude_cm", -1)

                # Drone near the ground or telemetry says 0 → cut motors
                if 0 <= alt <= LAND_ALTITUDE_THRESHOLD_CM:
                    break
                # Safety timeout
                if elapsed > MAX_DESCENT_S:
                    try: self.log_q.put_nowait("LAND: timeout — sending stop")
                    except queue.Full: pass
                    break

                _send_throttle(DESCENT_THROTTLE)
                time.sleep(1./self.rate)
        else:
            # ── timed ramp-down (no telemetry) ────────────────────────────
            ramp_steps = int(TIMED_RAMP_S * self.rate)
            thr_range = snap["throttle"] - STICK_MIN
            for i in range(ramp_steps):
                if not self._running: break
                frac = i / max(ramp_steps - 1, 1)
                thr = snap["throttle"] - frac * thr_range
                _send_throttle(max(STICK_MIN, thr))
                time.sleep(1./self.rate)

        # Final: send CMD_LAND (= 0x02) to cut motors cleanly
        for _ in range(10):
            c1,c2,c3 = self.state.next_counters()
            pkt = build_packet(STICK_MID, STICK_MID, STICK_MIN,
                               int(snap["yaw"]), CMD_LAND, headless, c1, c2, c3)
            self._send(pkt)
            time.sleep(0.02)
...
class SerialReader:
                ...
                if ser.in_waiting>0:
                    ...
                    if line:
                        ...
                        if not parsed and line.startswith("["):
                            try: self.log_q.put_nowait(line)
                            except queue.Full: pass
            except Exception as e:
    ...
    def _parse(self, line):
        m=self._telemetry_re.search(line)
        if not m:
            return False
        try:
            yaw=float(m.group(1)); pitch=float(m.group(2)); roll=float(m.group(3))
            thr=int(m.group(4))
            a0=int(m.group(5)); a1=int(m.group(6)); a2=int(m.group(7)); a3=int(m.group(8))
            pos=int(m.group(9)) if m.group(9) is not None else -1
            self.on_data(yaw,pitch,roll,thr,a0,a1,a2,a3,pos)
            return True
...
class GloveController:
    ...
    def reset_calibration(self):
        ...
        self._log("IMU: waiting for Arduino re-calibration telemetry…")

    def capture_zero(self):
        self._log("IMU: zero requested on Arduino")

    def on_sensor_data(self,yaw,pitch,roll,throttle,a0,a1,a2,a3,pos=-1):
        self.yaw_deg=yaw;self.pitch_deg=pitch;self.roll_deg=roll
        self.a2_raw=a2;self.a3_raw=a3
        self.nn_pos=pos

        thr=max(STICK_MIN,min(STICK_MAX,int(throttle)))
        self.throttle_pct=(thr-STICK_MID)/(STICK_MAX-STICK_MID)

        sy=self.mapper._a2s(yaw,   self.mapper.yaw_deadzone, self.mapper.yaw_sensitivity, self.mapper.yaw_expo)
        sp=self.mapper._a2s(pitch, self.mapper.pr_deadzone,  self.mapper.pr_sensitivity,  self.mapper.pr_expo)
        sr=self.mapper._a2s(roll,  self.mapper.pr_deadzone,  self.mapper.pr_sensitivity,  self.mapper.pr_expo)
        sticks={"throttle":float(thr),"yaw":sy,"pitch":sp,"roll":sr}

        if self.calibrating:
            self.calibrating=False
            self.calib_count=self.CALIB_SAMPLES
            self.flex_calibrated=True
            self._log("IMU calibrated on Arduino ✓")
...
class K417GUI:
    ...
    def _build_ui(self):
        ...
        self._build_connection(left);self._build_glove(left);self._build_keys_legend(left)
    ...
    def _build_sensitivity(self,parent):
        ...
        param("Deadzone (°)", 0.,30., 4., lambda v:setattr(self.glove.mapper,"yaw_deadzone",v), res=0.5, color=ACCENT2)
        ...
        tk.Label(f,text="Throttle is provided by Arduino telemetry (T)",
             fg=TEXT_DIM,bg=PANEL_BG,font=FONT_SMALL).pack(anchor="w",pady=(8,2))
    ...
    def _imu_zero(self):
        if self.serial and self.serial.send_command("O"):
            self.glove.capture_zero()
            self._log_event("IMU: zero command sent (O)")
        else:
            self._log_event("IMU: zero failed (serial not connected)")

    def _imu_recalib(self):
        if self.serial and self.serial.send_command("R"):
            self.glove.reset_calibration()
            self._log_event("IMU: re-calibration command sent (R)")
        else:
            self._log_event("IMU: re-calibration failed (serial not connected)")
    ...
    def _toggle_video(self):
        if self.video_adapter is not None:
            ...
            # ── START ─────────────────────────────────────────────────────
            # Use shared socket as required by many K417 variants
    ...
    def _tick(self):
        ...
        if g.calibrating:
            ...
            self._calib_status.config(text="● Waiting for Arduino calibration telemetry",fg=ACCENT2)
        else:
            self._calib_canvas.delete("all")
            self._calib_canvas.create_rectangle(0,0,140,10,fill=ACCENT3,outline="")
            self._calib_label.config(text="DONE ✓")
            self._calib_status.config(text="● Arduino IMU ready — press O to zero",fg=ACCENT3)
