...
FLIP_BURST_PACKETS = 16
FLIP_RECOVER_PACKETS = 8
FLIP_RECOVER_INSTA_PACKETS = 3
FLIP_THR_MIN = 165
FLIP_THR_BURST_BOOST = 28
FLIP_THR_RECOVER_BOOST = 26 #Change this
FLIP_THR_POST_BOOST = 8
FLIP_POST_HOLD_S = 0.25

# Flip tuning notes:
# - Original  values were:
#   BURST=20, RECOVER=10, THR_MIN=165, BURST_BOOST=28,
#   RECOVER_BOOST=20, POST_BOOST=22, POST_HOLD_S=0.40.
# - For most drones, keep packet counts fixed and tune only:
#   1) FLIP_THR_RECOVER_BOOST (main anti-drop strength)
#   2) FLIP_POST_HOLD_S (handoff duration)
# - Typical starting point for mixed E58 units:
#   RECOVER_BOOST=22, POST_HOLD_S=0.12
...
class IMUAxisMapper:
    ...
    THR_NET_DEADZONE=.25; THR_EXPO=.10; THR_NEUTRAL_SNAP_STICK=2.0

    def __init__(self):
        # Pitch + Roll controls
        self.pr_deadzone=11.; self.pr_sensitivity=1.; self.pr_expo=.5
        # Yaw controls (separate — glove movement feels very different)
        self.yaw_deadzone=16.; self.yaw_sensitivity=1.5; self.yaw_expo=.5
...
class DroneState:
    def __init__(self):
        ...
        # Simple post-flip throttle floor for a brief handoff.
        self._post_flip_hold_until = 0.0
        self._post_flip_hold_min = float(STICK_MIN)

    def set_imu(self, v):
        with self._lock:
            # Keep live sticks updating even during flips so UI state does not freeze.
            # The flip burst uses forced packet values in FlightController._do_flip().
    ...
    def arm_post_flip_hold(self, min_throttle: float, hold_seconds: float):
        with self._lock:
            self._post_flip_hold_until = time.time() + max(0.0, float(hold_seconds))
            self._post_flip_hold_min = max(float(STICK_MIN), min(float(STICK_MAX), float(min_throttle)))

    def clear_post_flip_hold(self):
        with self._lock:
            self._post_flip_hold_until = 0.0
            self._post_flip_hold_min = float(STICK_MIN)

    def enforce_post_flip_hold(self, throttle_value: int) -> int:
        with self._lock:
            if time.time() < self._post_flip_hold_until:
                return max(int(throttle_value), int(self._post_flip_hold_min))
...
class FlightController:
    ...
    def _loop(self):
        ...
        try:
            while self._running:
                ...
                if cmd != CMD_CALIBRATE and not self.state.flip_active:
                    throttle = self.state.enforce_post_flip_hold(throttle)
    ...
    def _do_flip(self):
        ...
        try:
            ...
            snap_thr = int(snap["throttle"])
            base_thr = max(snap_thr, FLIP_THR_MIN)
            ...
            for _ in range(FLIP_BURST_PACKETS):
                c1,c2,c3 = self.state.next_counters()
                pkt = build_packet(flip_roll, flip_pitch,
                                   burst_thr, int(snap["yaw"]),
                                   CMD_NONE, headless, c1, c2, c3,
                                   somersault_flag=True)
                self._send(pkt)
                time.sleep(1./self.rate)

            # Simple recovery: strong instant boost, then quick taper.
            insta_packets = min(FLIP_RECOVER_INSTA_PACKETS, FLIP_RECOVER_PACKETS)
            taper_packets = max(1, FLIP_RECOVER_PACKETS - insta_packets)
            den = max(1, taper_packets - 1)
            for i in range(FLIP_RECOVER_PACKETS):
                if i < insta_packets:
                    recover_thr = recover_insta_thr
                else:
                    t = (i - insta_packets) / den
                    recover_thr = int(round(recover_insta_thr + (recover_end_thr - recover_insta_thr) * t))
                c1,c2,c3 = self.state.next_counters()
                pkt = build_packet(STICK_MID, STICK_MID,
                                   recover_thr, int(snap["yaw"]),
                                   CMD_NONE, headless, c1, c2, c3)
                self._send(pkt)
                time.sleep(1./self.rate)
            self.state.arm_post_flip_hold(recover_end_thr, FLIP_POST_HOLD_S)
        finally:
    ...
    def stop(self):
        ...
        if self._thread:
            ...
            self._thread=None
        # Ensure flip state cannot remain latched across controller restarts.
        self.state.flip_active = False
        self.state.flip_dir = None
        self.state.clear_post_flip_hold()
...
class K417GUI:
    def __init__(self,root):
        ...
        self._nn_flip_mode=False
        self._nn_flip_trigger_latched=False
        self._nn_cmd_until=0.0
        self._nn_hold_s=0.35
        self._nn_cooldown_s=0.9
        self._nn_min_margin=0.05
        self._nn_min_margin_land=0.04
        self._nn_flip_mode_since=0.0
        self._flip_started_ts=0.0
        self._flip_max_active_s=2.5
    ...
    def _build_ui(self):
        ...
        r.minsize(1040,760)
        try:
            # Windows maximize on launch
            r.state("zoomed")
        except Exception:
            # Fallback: manually size to current screen
    ...
    def _build_sensitivity(self,parent):
        ...
        param("Deadzone (°)", 0.,30., 11., lambda v:setattr(self.glove.mapper,"pr_deadzone",v),  res=0.5, color=ACCENT3)
        ...
        param("Deadzone (°)", 0.,30., 16., lambda v:setattr(self.glove.mapper,"yaw_deadzone",v), res=0.5, color=ACCENT2)
        param("Sensitivity",  0.1,3., 1.5, lambda v:setattr(self.glove.mapper,"yaw_sensitivity",v),       color=ACCENT2)
    ...
    def _build_nn_cmd_panel(self,parent):
        ...
        self._flip_state_var=tk.StringVar(value="FLIP: IDLE")
        self._flip_state_label=tk.Label(
            f,
            textvariable=self._flip_state_var,
            fg=TEXT_DIM,
            bg=PANEL_BG,
            font=("Courier New",11,"bold"),
            anchor="w",
        )
        self._flip_state_label.pack(fill="x",pady=(0,4))
    ...
    def _trigger_nn_action(self, pred: int):
        if pred == 1:
            ...
            self._nn_flip_mode = True
            self._nn_flip_trigger_latched = False
            self._nn_flip_mode_since = time.time()
            self._set_nn_cmd_status("NN CMD: FLIP ARMED (one-shot)", IMU_COLOR)
        elif pred == 2:
            self._cmd_land()
    ...
    def _check_nn_flip_mode_trigger(self):
        if not self._nn_flip_mode:
            return

        # Auto-cancel stale armed mode so it cannot persist forever.
        if self._nn_flip_mode_since and (time.time() - self._nn_flip_mode_since) > 3.0:
            self._nn_flip_mode = False
            self._nn_flip_trigger_latched = False
            self._nn_flip_mode_since = 0.0
            self._set_nn_cmd_status("NN CMD: FLIP arm timeout", ACCENT2, hold_s=1.4)
            self._log_event("NN flip mode: timeout -> canceled")
            return
        ...
        snap = self.state.snapshot()
        roll = int(round(float(snap["roll"])))
        pitch = int(round(float(snap["pitch"])))

        if not self._nn_flip_trigger_latched:
            if roll == STICK_MAX:
                self._cmd_flip(FLIP_RIGHT)
                self._nn_flip_trigger_latched = True
                self._nn_flip_mode = False
                self._nn_flip_mode_since = 0.0
            elif roll == STICK_MIN:
                self._cmd_flip(FLIP_LEFT)
                self._nn_flip_trigger_latched = True
                self._nn_flip_mode = False
                self._nn_flip_mode_since = 0.0
            elif pitch == STICK_MAX:
                self._cmd_flip(FLIP_FORWARD)
                self._nn_flip_trigger_latched = True
                self._nn_flip_mode = False
                self._nn_flip_mode_since = 0.0
            elif pitch == STICK_MIN:
                self._cmd_flip(FLIP_BACKWARD)
                self._nn_flip_trigger_latched = True
                self._nn_flip_mode = False
                self._nn_flip_mode_since = 0.0

        if (self._nn_flip_trigger_latched and
                abs(roll - STICK_MID) <= 8 and
                abs(pitch - STICK_MID) <= 8):
            self._nn_flip_trigger_latched = False
    ...
    def _update_nn_logic(self):
        ...
        self._check_nn_flip_mode_trigger()
        ...
        req_margin = self._nn_min_margin_land if pred == 2 else self._nn_min_margin
    ...
    def _cmd_flip(self, direction: str):
        ...
        self._flip_started_ts = time.time()
    ...
    def _tick(self):
        ...
        if self.state.flip_active:
            self._flip_state_var.set("FLIP: ACTIVE")
            self._flip_state_label.config(fg=ACCENT2)
        elif self._nn_flip_mode:
            self._flip_state_var.set("FLIP: ARMED")
            self._flip_state_label.config(fg=IMU_COLOR)
        else:
            self._flip_state_var.set("FLIP: IDLE")
            self._flip_state_label.config(fg=TEXT_DIM)

        # Safety watchdog: if flip flag gets stuck (e.g., loop restarted mid-flip),
        # force-clear it so controls and NN logic can recover.
        if self.state.flip_active and self._flip_started_ts:
            if (time.time() - self._flip_started_ts) > self._flip_max_active_s:
                self.state.flip_active = False
                self.state.flip_dir = None
                self._flip_started_ts = 0.0
                self._log_event("FLIP: watchdog cleared stale flip flag")
