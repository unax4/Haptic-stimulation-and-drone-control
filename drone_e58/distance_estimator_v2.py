...
3. YOLOv8n-seg (COCO)  — when a person (≈ 1.75 m tall) is detected, its
   ...
   The segmentation head also produces a per-person contour mask drawn on
   the overlay instead of a plain rectangle.
...
class _ScaleCalibrator:
    ...
    def update(
        ...
        yolo_boxes: list | None,   # list of (x1,y1,x2,y2,cls,conf,contour)
    ) -> Optional[float]:
        """
    ...
    def _yolo_scale(
        ...
        best_area = 0
        for box in boxes:
            # Unpack safely — boxes are now (x1,y1,x2,y2,cls,conf,contour)
...
def _load_person_detector():
    """
    Load YOLOv8n-seg (COCO) as the person detector.

    Uses the segmentation variant so that each detection also returns a contour
    mask (drawn on the overlay) rather than just a bounding box.

    YOLOv8n-seg is chosen because:
      - ~7 MB, effectively same speed as yolov8n on CPU
      - COCO class 0 = person
      - masks.xy gives contour points in frame coordinates directly
      - No extra post-processing needed

    The returned model has one extra attribute:
      ._person_class_id  — always 0 for COCO
    """
    ...
    try:
        model = _YOLO("yolov8n-seg.pt")   # downloads ~7 MB on first run, cached after
        model._person_class_id = 0         # COCO class 0 = person
        print("[DistEst] YOLOv8n-seg (COCO) person detector loaded ✓")
        return model
...
class _PersonTracker:
    ...
    Box tuples are (x1, y1, x2, y2, cls, conf, contour) — the tracker stores
    and forwards the full tuple unchanged; IoU matching uses only the first 4
    coordinates so the contour field is transparent to it.
    """

    IOU_MATCH   = 0.20   # minimum IoU to associate a new detection with a track
    ...
    def update(self, raw_boxes: list) -> list:
        """
        raw_boxes: list of (x1,y1,x2,y2,cls,conf,contour) from this YOLO pass.
...
class DistanceEstimator:
    ...
    ----------
    use_yolo : bool
        Enable YOLOv8n-seg person-anchor (requires ultralytics).
    yolo_every_n : int
        Run YOLO only every N frames (default 1) to catch persons at low fps.
    draw_overlay : bool
    ...
    def _preprocess_for_yolo(self, frame: np.ndarray) -> np.ndarray:
        """
        Fast JPEG artifact reduction for YOLO inference.

        Bilateral filter is replaced with a small Gaussian blur — ~20× faster
        and sufficient to smooth JPEG block boundaries without destroying edges.
        CLAHE is skipped here for speed; contrast is rarely the limiting factor
        once block artifacts are removed.
        """
    ...
    def _draw_osd(
        ...
        # ── 2. Person detections — contour mask or bbox fallback ──────────────
        if boxes:
            # Build all filled masks in a single layer then blend once —
            # avoids one full-frame addWeighted call per person.
            has_contour = any(
                len(box) > 6 and box[6] is not None and len(box[6]) >= 3
                for box in boxes
            )
            if has_contour:
                mask_layer = out.copy()
                for box in boxes:
                    x1, y1, x2, y2, cls, conf, *extra = box
                    contour = extra[0] if extra else None
                    if contour is not None and len(contour) >= 3:
                        cv2.fillPoly(mask_layer, [contour.reshape((-1, 1, 2))], GREEN)
                cv2.addWeighted(mask_layer, 0.25, out, 0.75, 0, out)  # single blend
            ...
            for box in boxes:
                ...
                # Distance label anchored to top-left of bbox
                pcx = np.clip(int((x1 + x2) / 2 * dw / w), 0, dw - 1)
                pcy = np.clip(int((y1 + y2) / 2 * dh / h), 0, dh - 1)
                ...
                lx, ly = int(x1), max(int(y1) - 6, lh + 4)
    ...
    CONF_THRESHOLD  = 0.20   # low — temporal tracker is the false-positive guard
    MIN_BOX_PX      = 8      # minimum box dimension in pixels
    MIN_AREA_PX2    = 80     # minimum box area
    YOLO_IMGSZ      = 320    # inference resolution fed to YOLO (ultralytics handles
                             # resize+letterbox internally — no manual upscale needed)

    def _run_yolo(self, frame: np.ndarray) -> list:
        """
        Run YOLOv8n-seg and return temporally confirmed detections.

        Each confirmed detection is a tuple:
            (x1, y1, x2, y2, cls=0, conf, contour)
        where contour is an (N,2) int32 array of polygon points in the
        ORIGINAL frame's coordinate space, or None if the mask was unavailable.

        Speed strategy:
          - imgsz=320 passed to ultralytics: it handles resize+letterbox
            internally and maps outputs back to original coords automatically.
            No manual Python resize needed — saves one full frame copy.
          - Small Gaussian blur instead of bilateral for JPEG artifact removal.
          - yolo_every_n controls how often this runs; tracker holds detections
            between passes so the overlay never flickers.
        """
        # ── fast JPEG artifact smoothing ──────────────────────────────────────
        proc = self._preprocess_for_yolo(frame)

        try:
            person_cls = self._yolo._person_class_id
            results = self._yolo(proc, verbose=False,
                                 conf=self.CONF_THRESHOLD,
                                 classes=[person_cls],
                                 imgsz=self.YOLO_IMGSZ)
            raw = []
            for r in results:
                masks = r.masks   # None when no detections
                for i, b in enumerate(r.boxes):
                    # ultralytics returns coords in ORIGINAL frame space
                    ...
                    # masks.xy is also already in original frame coords
                    contour = None
                    if masks is not None and i < len(masks):
                        pts = masks.xy[i]   # (N,2) float32
                        if len(pts) >= 3:
                            contour = pts.astype(np.int32)

                    raw.append((x1, y1, x2, y2, 0, conf, contour))
