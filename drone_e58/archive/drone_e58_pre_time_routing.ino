...
void updateHapticFeedback(uint8_t stickYaw, uint8_t stickPitch, uint8_t stickRoll, uint8_t stickThrottle) {
  ...
  uint16_t hvCombined = 0x0000;
  if (yawActive) hvCombined |= positions[hapticYaw.position];
  if (pitchActive) hvCombined |= positions[hapticPitch.position];
  if (rollActive) hvCombined |= positions[hapticRoll.position];
  if (throttleActive) hvCombined |= positions[hapticThrottle.position];
  ...
  // Shared potentiometer: pick strongest requested intensity (lowest pot code).
  int potVal = 255;
  if (yawActive) potVal = min(potVal, yawPot);
  if (pitchActive) potVal = min(potVal, pitchPot);
  if (rollActive) potVal = min(potVal, rollPot);
  if (throttleActive) potVal = min(potVal, throttlePot);

  hapticSendToHV2701(hvCombined);
  hapticSetPot((byte)constrain(potVal, 0, 255));
